...
(function() {
  ...
  this.read_tidal_and_lunar_event_batches = function(route, handler) {
    ...
    * finally, the lunar events—phases as well as declination and distance extrema—are sent.
     ...
     */
    var lunar_event_batch, tasks, tidal_extrema_event_batch, tidal_hl_event_batch;
        ...
        return _this._walk_tidal_and_lunar_phase_events(route, function(error, event) {
          ...
          switch (types = TYPES.type_of(event)) {
            case 'TIDES/tidal-extrema-event':
              return tidal_extrema_event_batch.push(event);
    ...
    ASYNC.parallel(tasks, (function(_this) {
      return function(error) {
        ...
        return handler(null, [tidal_extrema_event_batch, tidal_hl_event_batch, lunar_event_batch]);
      };
    })(this));
    return null;
  };
  ...
  this.read_aligned_events = function(route, handler) {
    this.read_tidal_and_lunar_event_batches(route, (function(_this) {
      return function(error, event_batches) {
        var lunar_event, lunar_event_batch, tidal_extrema_event_batch, tidal_hl_event_batch, _i, _len;
        if (error != null) {
          throw error;
        }
        tidal_extrema_event_batch = event_batches[0], tidal_hl_event_batch = event_batches[1], lunar_event_batch = event_batches[2];
        for (_i = 0, _len = lunar_event_batch.length; _i < _len; _i++) {
          lunar_event = lunar_event_batch[_i];
          _this._align_lunar_with_tidal_event(tidal_hl_event_batch, lunar_event);
        }
