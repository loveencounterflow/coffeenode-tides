...
(function() {
  var BAP, TEX, TIDES, TRM, TYPES, alert, badge, cline, compile_options, container, d, debug, echo, error, esc, eventually, hbox, help, hline, hrule, info, key, leading, leavevmode, locator, locators, log, multicolumn, newline, next_cell, next_line, njs_fs, njs_path, options, postscript, preamble, read, rpr, thinspace, value, warn, whisper, _i, _len, _ref,
    __slice = [].slice;
  ...
  info(BAP.walk_containers_crumbs_and_values(options, function(error, container, crumbs, value) {
    if (error != null) {
      throw error;
    }
    if (crumbs === null) {
    ...
    return log('', TRM.gold('/' + (crumbs.join('/'))), TRM.lime(rpr(value)));
  }));
  ...
  BAP.walk_containers_crumbs_and_values(d, function(error, container, crumbs, value) {
  ...
  locators = ['/meaningless/0', '/meaningless/1', '/meaningless/2/foo', '/meaningless/2/bar', '/meaningless/2/nested/0', '/meaningless/2/nested/1', '/meaningless/3', '/deep/down/in/a/drawer', '/deep/down/in/a/cupboard', '/deep/down/in/a/box'];

  for (_i = 0, _len = locators.length; _i < _len; _i++) {
    locator = locators[_i];
    _ref = BAP.container_and_facet_from_locator(d, locator), container = _ref[0], key = _ref[1], value = _ref[2];
    info(locator, TRM.grey(locator), TRM.gold(key), rpr(value));
  }

  log(BAP.container_and_facet_from_locator(42, '/'));

  compile_options = function(options) {
    return BAP.walk_containers_crumbs_and_values(options, (function(_this) {
