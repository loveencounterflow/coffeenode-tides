...
(function() {
  var FI, GM, TIDES, TRM, TYPES, WRITE_TEX, alert, badge, debug, echo, f, help, info, log, njs_fs, njs_path, options, rpr, warn, whisper, x;
  ...
  module.exports = this._draw_curves_with_gm = function(route, raw_dots, handler) {
     ...
     */
    var bezier_h_dots, bezier_h_points, bezier_hl_points, bezier_l_dots, bezier_l_points, black, blue, collection, color, day, dots, hl, hl3, idx, image, last_idx, name, names, names_and_colors, point, points, red, row_idx, x0, x1, y0, y1, _i, _j, _k, _l, _len, _len1, _len2, _len3, _len4, _len5, _len6, _m, _n, _o, _ref, _ref1, _ref2, _ref3, _ref4, _ref5, _ref6, _ref7, _ref8;
    bezier_hl_points = [];
    bezier_h_dots = [];
    bezier_l_dots = [];
    bezier_h_points = [];
    ...
    for (_j = 0, _len1 = names_and_colors.length; _j < _len1; _j++) {
      ...
      for (_k = 0, _len2 = names.length; _k < _len2; _k++) {
        ...
        y0 = this._image_px_from_y_raw(0);
        y1 = this._image_px_from_y_raw(60);
        image.stroke(color, 2).drawLine(x0, y0, x1, y1);
      }
    }
    ...
    for (idx = _l = 0, _len3 = bezier_hl_points.length; _l < _len3; idx = ++_l) {
      ...
      x1 = this._image_px_from_origin_mm((hl === 'h' ? 34 : 46) + 1);
      y0 = y1 = points[0][1];
      image.stroke(black, 1).drawLine(x0, y0, x1, y1);
      if (idx === last_idx) {
        hl3 = hl === 'h' ? 'l' : 'h';
        x0 = points[3][0];
        x1 = this._image_px_from_origin_mm((hl3 === 'h' ? 34 : 46) + 1);
        y0 = y1 = points[3][1];
        image.stroke(black, 1).drawLine(x0, y0, x1, y1);
      }
    }
    ...
    for (_m = 0, _len4 = _ref4.length; _m < _len4; _m++) {
      _ref5 = _ref4[_m], row_idx = _ref5.row_idx, day = _ref5.day, hl = _ref5.hl;
      x0 = this._image_px_from_origin_mm(10);
      if (hl === 'h') {
        x1 = this._image_px_from_origin_mm(35);
      } else {
        x1 = this._image_px_from_origin_mm(23);
      }
      y0 = y1 = this._image_px_from_y_raw(row_idx - 1);
      color = day === 1 ? red : black;
      image.stroke(color).drawLine(x0, y0, x1, y1);
      x0 = x1;
      y0 = y1;
      y1 += this._image_px_from_y_raw(0.27);
      image.stroke(color).drawLine(x0, y0, x1, y1);
      if (hl === 'l') {
        x0 = x1;
        y0 = y1;
    ...
    _ref6 = [bezier_h_points, bezier_l_points];
    for (_n = 0, _len5 = _ref6.length; _n < _len5; _n++) {
      collection = _ref6[_n];
      for (idx = _o = 0, _len6 = collection.length; _o < _len6; idx = ++_o) {
        _ref7 = collection[idx], hl = _ref7[0], points = _ref7[1];
        (_ref8 = image.stroke(black, 1)).drawBezier.apply(_ref8, points);
      }
    }
