...
(function() {
  var GM, TIDES, TRM, TYPES, alert, badge, debug, echo, f, help, info, log, njs_fs, njs_path, options, rpr, warn, whisper, x;
  ...
  module.exports = this._draw_curves_with_gm = function(route, raw_dots, handler) {
     ...
     */
    var bezier_h_dots, bezier_h_points, bezier_hl_points, bezier_l_dots, bezier_l_points, collection, dots, hl, idx, image, name, point, points, x0, x1, y0, y1, _i, _j, _k, _l, _len, _len1, _len2, _len3, _len4, _m, _ref, _ref1, _ref2, _ref3, _ref4, _ref5, _ref6;
    bezier_hl_points = [];
    bezier_h_dots = [];
    bezier_l_dots = [];
    bezier_h_points = [];
    ...
    _ref1 = 'max-l-height min-h-height max-h-height'.split(/\s+/);
    for (_j = 0, _len1 = _ref1.length; _j < _len1; _j++) {
      name = _ref1[_j];
      x0 = x1 = this._image_px_from_real_cm(TIDES['options']['data']['tides'][name]);
      y0 = this._image_px_from_y_raw(0);
      y1 = this._image_px_from_y_raw(60);
      image.stroke('red', 1).drawLine(x0, y0, x1, y1);
    }
    ...
    y1 = this._image_px_from_y_raw(60);
    image.stroke('black', 1).drawLine(x0, y0, x1, y1);
    for (idx = _k = 0, _len2 = bezier_hl_points.length; _k < _len2; idx = ++_k) {
      ...
      x1 = this._image_px_from_origin_mm((hl === 'h' ? 40 : 75) + 1);
      y0 = y1 = points[0][1];
      image.stroke('black', 1).drawLine(x0, y0, x1, y1);
    }
    ...
    for (_l = 0, _len3 = _ref4.length; _l < _len3; _l++) {
      collection = _ref4[_l];
      for (idx = _m = 0, _len4 = collection.length; _m < _len4; idx = ++_m) {
